...
public class GestureAnalyser {
    ...
    public GestureType getGesture(MotionEvent ev) {
        ...
        // Additional check for pinch and unpinch
        if (gt.getGestureFlag() == 0 && numFingers >= 2) {
            double initialDistance = initialFingDist(0, 1);
            double finalDistance = finalFingDist(0, 1);

            if (finalDistance > 1.5 * initialDistance) {
                gt.setGestureFlag(UNPINCH_2);
            } else if (finalDistance < 0.8 * initialDistance) {
                gt.setGestureFlag(PINCH_2);
            }
        }
